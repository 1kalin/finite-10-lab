...
function safeUrl(value, name) {
  ...
  if (hostname === 'localhost' || hostname.endsWith('.localhost') || hostname.endsWith('.local') ||
      (ipKind === 4 && isPrivateIpv4(hostname)) || (ipKind === 6 && isPrivateIpv6(hostname))) {
...
export class ModerationWorkflow {
  ...
  submit(input, actorId) {
    ...
    if (data.ownerId !== actor) throw new Error('ownerId must match the authenticated actor');
  ...
  edit(id, changes, actorId) {
    ...
    if (changes && Object.prototype.hasOwnProperty.call(changes, 'ownerId') && changes.ownerId !== record.ownerId) {
      throw new Error('ownerId cannot be changed');
    }
